...
const Cart = ({ dispatch }) => {
  ...
  const handlePaymentSuccess = useCallback(async (response) => {
    ...
    try {
      const verifyResponse = await axios.post('https://oibsip-tasty-pizza.onrender.com/api/verify-payment', response, {
        headers: {
      ...
      if (verifyResponse.data.success) {
        ...
        const data = await axios.post('https://oibsip-tasty-pizza.onrender.com/api/order-details', orderDetailsData, {
          headers: {
            Authorization: `Bearer ${token}`
          }
        });
        ...
        if (data.status === 201) {
          ...
          try {
            await axios.put(
              'https://oibsip-tasty-pizza.onrender.com/api/pizza-update',
              {
                cartItems: cartItems.map(item => ({
                  pizzaId: item.pizzaId,
